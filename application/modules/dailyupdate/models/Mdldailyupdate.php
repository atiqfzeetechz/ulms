...
function get_message_history($limit,$start,$filters = [])
 ...
 function get_insta_gellery($limit, $start) {
    ...
    foreach ($query as $row) {
        ...
        $del_url = base_url().'dailyupdate/delete_insta_gellery/'.$common_key;
        $delete_link = "<a href='javascript:void(0)' onclick='return delete_me(this.id);' id='$del_url'><i class='fa fa-trash'></i></a>";
           
      

        // View button with onclick to fetch media
        $view_btn = "<a href='javascript:void(0)' onclick='viewMedia(\"$common_key\")'><i class='fa fa-eye'></i></a>";
        if($row->status == 1){
            $bg = '#dcdcdc';
        }
        else{
        ...
        if($row->added!='0000-00-00 00:00:00'){
          $up = date('Y-m-d',strtotime($row->added));
        }
        else{
          $up = '';
