...
$(this).autocomplete({
     source: function( request, response ) {
		...
		$.ajax({
			url : 'http://localhost/stock/party/getSeacrhParty_Customer/',
			dataType: "json",
			method: 'post',
			data: {
...
function countTotalAmt(){
		    ...
		    $('#dynemic_no_gen tr#rec-1').each(function() {
				var SalesPrice              = $("#SalesPrice_"+i).val();
				var Qty                        = $("#ItemQty_"+i).val();
				var ItemAmountQtyMrp = SalesPrice*Qty;
...
$(".loading").show();
var post_link = "<?php echo base_url(); ?>transaction/saveSales/save";
